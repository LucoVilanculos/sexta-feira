...
const validateDashboardSettings = [
  body("dashboard.layout").optional().isIn(["grid", "list"]).withMessage("Layout inválido"),
  body("dashboard.defaultView").optional().isIn(["day", "week", "month"]).withMessage("Visualização padrão inválida"),
  body("dashboard.widgets").optional().isArray().withMessage("Widgets deve ser um array"),
]

const validateCalendarSettings = [
  body("calendar.defaultView").optional().isIn(["day", "week", "month"]).withMessage("Visualização padrão inválida"),
  body("calendar.weekStartsOn").optional().isIn([0, 1, 6]).withMessage("Dia inicial da semana inválido"),
  body("calendar.showWeekends").optional().isBoolean().withMessage("showWeekends deve ser um booleano"),
  ...
  body("calendar.defaultReminders").optional().isArray().withMessage("Lembretes padrão deve ser um array"),
]
...
const validateNotificationSettings = [
  ...
  body("notifications.push").optional().isObject().withMessage("Configurações de push devem ser um objeto"),
  body("notifications.desktop").optional().isObject().withMessage("Configurações de desktop devem ser um objeto"),
]

const validateThemeSettings = [
  body("theme.mode").optional().isIn(["light", "dark", "system"]).withMessage("Modo inválido"),
  ...
  body("theme.fontSize").optional().isIn(["small", "medium", "large"]).withMessage("Tamanho de fonte inválido"),
  body("theme.fontFamily").optional().isString().withMessage("Família de fonte deve ser uma string"),
]

const validateWidgetUpdate = [
  body("type").optional().isIn(["calendar", "tasks", "projects", "weather", "notes", "clock"]).withMessage("Tipo de widget inválido"),
